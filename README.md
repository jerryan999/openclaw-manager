...
**CI 自动构建**：推送版本 tag（如 `v0.0.8`）后，[GitHub Actions](.github/workflows/build.yml) 会自动同时构建 macOS 与 Windows 版本，并创建 **GitHub Release**，安装包可直接在 [Releases](https://github.com/miaoxworld/openclaw-manager/releases) 页面下载。
...
- 预装 OpenClaw 运行时：`src-tauri/resources/offline/npm-global.zip`（由 GitHub Actions 在 Windows 构建时自动生成，内含 OpenClaw + 通过 clawhub 预装的 sonoscli）
